---
title: "Tentang Kami"
description: "Membangun infrastruktur digital yang andal sejak 1985."
hero:
  heading: "Membangun Infrastruktur Digital yang Andal Sejak 1985"
  tagline: "Tentang Kami"
  image: "/img/about-hero.jpg"
intro: >-
  Selama lebih dari empat dekade, PT. Limawira Wisesa telah menjadi perusahaan
  engineering dan system integrator tepercaya, menghadirkan solusi Critical
  Power, Data Center, Infrastruktur ICT, dan Teknik Elektrikal yang andal
  bagi organisasi di seluruh Indonesia.
story:
  - year: "1985"
    title: "Berdiri di Jakarta"
    description: >-
      Didirikan oleh lima profesional muda dengan keahlian di bidang Teknik
      Mesin dan Elektrikal, Limawira Wisesa memulai perjalanannya dengan
      menyediakan layanan engineering berkualitas tinggi. Seiring dengan
      perkembangan teknologi yang pesat dan tuntutan pelanggan yang semakin
      kompleks, perusahaan terus mengembangkan kapabilitasnya untuk
      menghadirkan solusi infrastruktur terintegrasi yang mendukung bisnis
      modern.
  - year: "1990"
    title: "Memasuki Industri Critical Power"
    description: >-
      Limawira Wisesa memasuki industri Critical Power dan menjadi distributor
      eksklusif sistem Uninterruptible Power Supply (UPS) Exide Electronics USA
      di Indonesia. Tonggak ini membangun reputasi kuat perusahaan dalam
      menghadirkan solusi proteksi daya yang andal untuk lingkungan
      mission-critical.
  - year: "Pertengahan 1990-an"
    title: "Infrastruktur ICT"
    description: >-
      Menyadari meningkatnya permintaan akan sistem informasi dan komunikasi
      terintegrasi, Limawira Wisesa memperluas keahliannya ke Infrastruktur ICT
      dengan memperkenalkan solusi enterprise networking dan structured cabling
      dari produsen global terkemuka, termasuk Allied Telesyn, Panduit,
      Alcatel, dan AdventNet. Ekspansi strategis ini memungkinkan perusahaan
      menyediakan solusi end-to-end yang lengkap bagi pelanggan, bukan sekadar
      produk individual.
  - year: "2005"
    title: "Divisi R&D Internal"
    description: >-
      Didorong oleh komitmen terhadap inovasi, Limawira Wisesa membangun Divisi
      Penelitian & Pengembangan sendiri dengan workshop dan fasilitas
      pengujian khusus. Investasi ini memungkinkan perusahaan merancang serta
      memproduksi sistem proteksi elektrikal khusus sesuai kebutuhan unik
      pelanggan, sambil berkolaborasi dengan mitra OEM internasional dari
      negara-negara industri terkemuka.
  - year: "Saat Ini"
    title: "System Integrator Menyeluruh"
    description: >-
      Saat ini, Limawira Wisesa telah berkembang menjadi System Integrator
      menyeluruh, menghadirkan solusi end-to-end yang mencakup konsultasi,
      desain engineering, pengadaan, instalasi, pengujian & commissioning,
      pemeliharaan, hingga layanan managed support. Keahlian kami meliputi
      Data Center, Sistem Critical Power, Infrastruktur ICT, Teknologi Smart
      Building, Teknik Elektrikal, dan Preventive Maintenance — membantu
      organisasi membangun infrastruktur yang aman, efisien, dan siap untuk
      masa depan.
vision:
  title: "Visi"
  text: "Menjadi yang terbaik dalam menyediakan layanan dan dukungan di bidang keahlian kami."
mission:
  title: "Misi"
  items:
    - "Menyediakan solusi terbaik di bidang Sistem Informasi, Komunikasi Data, dan teknologi Power Supply."
    - "Mencapai profitabilitas yang berkelanjutan dan wajar bagi perusahaan serta pemegang saham."
    - "Meningkatkan kesejahteraan dan well-being seluruh karyawan kami."
values:
  - title: "Keunggulan Engineering"
    description: "Setiap proyek dipimpin oleh engineer bersertifikat dan diaudit sesuai standar internasional."
  - title: "Keberhasilan Pelanggan"
    description: "Kami mengukur keberhasilan kami dari hasil jangka panjang yang kami berikan kepada pelanggan."
  - title: "Inovasi"
    description: "Divisi R&D internal kami menjaga kami di garis depan infrastruktur elektrikal dan digital."
  - title: "Integritas"
    description: "Kami melakukan yang benar, bahkan ketika tidak ada yang melihat."
stats:
  - label: "Tahun pengalaman"
    value: "40+"
  - label: "Proyek selesai"
    value: "500+"
  - label: "Pelanggan puas"
    value: "200+"
  - label: "Ketersediaan dukungan"
    value: "24/7"
cta:
  heading: "Siap bekerja sama dengan kami?"
  text: "Ceritakan proyek Anda dan tim kami akan menghubungi Anda dalam satu hari kerja."
  label: "Hubungi kami"
  url: "/contact/"
---
